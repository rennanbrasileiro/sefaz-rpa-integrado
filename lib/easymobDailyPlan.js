...
function step(key, reference, calculated, status, actual, canExecute, op, reason) {
  return { key, referenceTime: reference.label, calculatedTime: minutesToHHMM(calculated ?? reference.minutes), status: status === 'ready' ? 'pendente' : status, rawStatus: status, actualMark: actual?.raw || null, canExecute: Boolean(canExecute), willWrite: Boolean(canExecute && op?.willWrite), reason };
}
