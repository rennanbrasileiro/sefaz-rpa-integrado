...
async function easyAuthorizeRealToday(){ const ok=await askConfirm('Autorizar execução REAL automática até 23:59 de hoje? TESTE continua sem gravar; REAL só age se o plano permitir.',{title:'Autorizar REAL hoje',ok:'Autorizar hoje'}); if(!ok)return; setBusy('easyApprovalBox',true); try{ const r=await api('/automation/approval/authorize',{method:'POST',body:JSON.stringify({until:endOfTodayIso()})}); set('easyRealApproval','true'); set('easyApprovalStatus',approvalSummary(r.validUntil)); toast('REAL autorizado até 23:59 de hoje.','warn'); await refreshCentralState(); }catch(e){ toast('Erro ao autorizar REAL: '+e.message,'danger'); } finally{ setBusy('easyApprovalBox',false); } }
async function easyRevokeReal(){ const ok=await askConfirm('Revogar autorização REAL de hoje? A rotina continuará em TESTE ou ficará bloqueada para gravação real.',{title:'Revogar autorização REAL',ok:'Revogar'}); if(!ok)return; try{ await api('/automation/approval/revoke',{method:'POST'}); set('easyRealApproval','false'); set('easyApprovalStatus','REAL não autorizado hoje'); toast('Autorização REAL revogada.'); await refreshCentralState(); }catch(e){ toast('Erro ao revogar REAL: '+e.message,'danger'); } }
async function refreshScheduler(){ try{ const r=await api('/automation/scheduler/status'); if($('schedulerBadge')) $('schedulerBadge').textContent=r.installed?'instalado':r.status; if($('schedulerStatus')) $('schedulerStatus').innerHTML=`<div class="metric-row"><div><span>Tarefa</span><b>${r.taskName||'--'}</b></div><div><span>Status</span><b>${r.status||'--'}</b></div><div><span>Instalada</span><b>${r.installed?'Sim':'Não'}</b></div><div><span>Plataforma</span><b>${r.status==='unsupported_non_windows'?'não Windows':'Windows'}</b></div></div><div class="hint">Scripts: ${Object.values(r.scripts||{}).join(' · ')}</div>`; }catch(e){ if($('schedulerStatus')) $('schedulerStatus').innerHTML=`<div class="alert danger">${e.message}</div>`; } }
async function installScheduler(){ const r=await api('/automation/scheduler/install',{method:'POST'}); toast(r.ok?'Tarefa instalada.':(r.message||'Instalação indisponível neste ambiente'), r.ok?'ok':'warn'); refreshScheduler(); }
async function removeScheduler(){ const r=await api('/automation/scheduler/remove',{method:'POST'}); toast(r.ok?'Tarefa removida.':(r.message||'Remoção indisponível neste ambiente'), r.ok?'ok':'warn'); refreshScheduler(); }
async function testScheduler(){ const r=await api('/automation/scheduler/test',{method:'POST'}); toast(r.ok?'Teste iniciado.':(r.message||'Teste indisponível neste ambiente'), r.ok?'ok':'warn'); refreshScheduler(); }
